    ...
    function render(maze, path) {
        ...
        for (y = 0; y < maze.length; y++) {
            ...
            for (x = 0; x < row.length; x++) {
                ...
                var rowInnerElem = element('div', 'maze__cell' + (type ? ' maze__cell_' + type : ''));

                //показываем первоначальные значения ячеек для наглядности работы алгоритма
                root.showDefaultCellValue(rowInnerElem, maze, x, y);
